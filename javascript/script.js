...
(function() {
    ...
    function createErrorWindow(x, y, msg) {
        ...
        win.innerHTML = `
            <div class="error-titlebar">
                <span>${msg || '⚠️ Error'}</span>
                <span>✕</span>
            </div>
            <div class="error-content">
                ❌ Critical error
                <div class="error-buttons"><div class="error-ok">OK</div></div>
            </div>`;
    ...
    function showEarlyMessage() {
        ...
        msg.textContent = 'No challenge available yet!';
    ...
    function createQuestionWindow() {
        const q = window.quizDatabase?.['1'] || {
            question: 'Error', correctAnswer: 'no', explanationWrong: 'Unknown error'
        };
    ...
    function createPasswordWindow() {
        ...
        win.innerHTML = `
            <div class="window-titlebar">
                <span>Password Challenge</span>
                <div class="window-controls"><button class="close-btn">✕</button></div>
            </div>
            <div class="window-content">
                <div class="password-requirements">
                    <p>Min length: <strong>8</strong> &nbsp;|&nbsp;
                       Numbers: <strong>${numbers}</strong> &nbsp;|&nbsp;
                       Capitals: <strong>${capitals}</strong> &nbsp;|&nbsp;
                       Special chars: <strong>${special}</strong></p>
                    <p style="margin-top:6px;color:#555;font-size:0.9rem;">No spaces allowed.</p>
                </div>
                <div style="margin:18px 0 12px;position:relative;">
                    ...
                    <button id="toggleButton"
                            style="position:absolute;right:8px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:0.85rem;color:#555;">Show</button>
                </div>
                <button id="submitButton"
                        style="width:100%;padding:11px;font-size:1rem;cursor:pointer;border:1px solid #aaa;border-radius:4px;background:#e8e4d0;">Verify</button>
            </div>`;
        ...
        tb.addEventListener('click', () => {
            ...
            tb.textContent = h ? 'Hide' : 'Show';
        });

        win.querySelector('#submitButton').addEventListener('click', () => {
            const p = pf.value, errs = [];
            if (p.includes(' ')) errs.push('No spaces allowed');
            if (p.length < 8)   errs.push(`Need 8+ characters (got ${p.length})`);
            const nc = (p.match(/[0-9]/g)||[]).length;
            if (nc < numbers)   errs.push(`Need ${numbers} number(s) (got ${nc})`);
            const cc = (p.match(/[A-Z]/g)||[]).length;
            if (cc < capitals)  errs.push(`Need ${capitals} capital(s) (got ${cc})`);
            const sc = (p.match(/[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/g)||[]).length;
            if (sc < special)   errs.push(`Need ${special} special(s) (got ${sc})`);
            ...
            if (errs.length === 0) {
                ...
                if (notificationFolder2) notificationFolder2.style.display = 'none';
                triggerGameOver('❌ Invalid password: ' + errs.join('. '));
            }
        });
    ...
    function startCallChallenge(callWin, panelWin) {
        ...
        cs.timerID = setInterval(() => {
            ...
            if (cs.secs <= 0) {
                clearInterval(cs.timerID);
                cleanupCall();
                triggerGameOver('❌ Time ran out! You were too slow.');
            }
        }, 1000);
    }
    ...
    function handleColorButton(color) {
        ...
        if (correct) {
            ...
            triggerGameOver(`❌ Wrong button! You pressed ${pName}, but they said ${tName}.`);
        }
    }
    ...
    function createInstructionsWindows() {
        ...
        callWin.innerHTML = `
            ...
            <div class="call-screen ${isSafe ? 'safe' : 'danger'}">
                ...
                <div class="call-status">${isSafe ? 'Azerbaijan number' : 'Unknown foreign number'}</div>
                <div class="call-buttons">
                    <button class="call-btn accept"  title="Accept">✔</button>
                    <button class="call-btn decline" title="Decline">✖</button>
                </div>
            </div>`;
        ...
        panelWin.innerHTML = `
            ...
            <div class="btn-panel-content">
                <button class="color-btn red    locked" data-color="red">Red</button>
                <button class="color-btn blue   locked" data-color="blue">Blue</button>
                <button class="color-btn green  locked" data-color="green">Green</button>
                <button class="color-btn yellow locked" data-color="yellow">Yellow</button>
            </div>`;
        ...
        callWin.querySelector('.call-btn.accept').addEventListener('click', () => {
            if (isSafe) {
                ...
                if (notificationFolder3) notificationFolder3.style.display = 'none';
                cleanupCall(); callWin.remove(); panelWin.remove();
                triggerGameOver('❌ You accepted a call from an unknown foreign number! Virus installed.');
            }
        });
        ...
        callWin.querySelector('.call-btn.decline').addEventListener('click', () => {
            if (!isSafe) {
                ...
                if (notificationFolder3) notificationFolder3.style.display = 'none';
                cleanupCall(); callWin.remove(); panelWin.remove();
                triggerGameOver('❌ You declined a safe Azerbaijan call! Mission failed.');
            }
        });
    ...
    function createEmptyWindow(folderId, msg) {
        const names = { '1':'Links', '2':'Password', '3':'Instructions' };
        const win = document.createElement('div');
        win.className    = 'window';
        win.style.left   = 200 + Math.random() * 200 + 'px';
        win.style.top    = 150 + Math.random() * 150 + 'px';
        ...
        win.innerHTML = `
            <div class="window-titlebar">
                <span>${names[folderId] || 'Folder'}</span>
                <div class="window-controls"><button class="close-btn">✕</button></div>
            </div>
    ...
    folderIcons.forEach(icon => {
        icon.addEventListener('dblclick', e => {
            ...
            if (id === '1') {
                ...
                else { createEmptyWindow(id, 'No challenge here right now. Wait for a notification.'); showEarlyMessage(); }
                ...
                else { createEmptyWindow(id, 'No call incoming right now. Wait for a notification.'); showEarlyMessage(); }
            }
        });
    });
